...
- [ ] **Smart Search Indexing**: Update SQLite schema and indexer to include a `smart_text` field that intelligently combines body content and metadata values (filtering out noise like property names and dates) for better search relevance.
- [ ] **Collaborative Search Design**: Refine the database schema for properties, tags, and dates based on the design document in `docs/design/search-optimization.md`.
...
- [ ] **Read-only View Mode**: Implement a "View Mode" for notes (potentially using the Toast UI Viewer) with an **Edit** button to toggle into the full editor.
