...
export const ObjectsController = {
    ...
    create: async (req) => {
        try {
            const { title: reqTitle, type: reqType, templateId, variables } = await req.json();
            ...
            if (templateId) {
                ...
                if (template) {
                    ...
                    // Merge metadata, filtering out system properties (starting with _)
                    const filteredTemplateMeta = Object.fromEntries(
                        Object.entries(templateMeta).filter(([k]) => !k.startsWith('_'))
                    );

                    metadata = { 
                        ...filteredTemplateMeta,
                        ...metadata,
                        type: reqType || filteredTemplateMeta.creates || filteredTemplateMeta.type || 'note'
                    };
                    metadata.title = reqTitle; // Hard override with the new title

                    // 1. Prepare all possible substitutions
                    const substitutions = {
                        ...filteredTemplateMeta, // Original template props (filtered)
                        ...variables,            // User-provided variables
                        'title': reqTitle,
                        'date': dateStr
                    };
                    console.log("DEBUG: reqTitle:", reqTitle);
                    console.log("DEBUG: substitutions:", JSON.stringify(substitutions));

                    // 2. Perform substitutions in Content and Metadata
                    // Note: 'title' and 'date' in substitutions will overwrite any template-specific title/date
                    for (const [key, val] of Object.entries(substitutions)) {
                        ...
                        // Direct metadata override if key exists in variables
                    ...
                    metadata.title = reqTitle; // Re-ensure title is correct after loop
