 ...
 * @param {string} context - 'client' or 'server'
 */
export function createDvLibrary(context = 'client') {
    const formatValue = (val) => {
        if (val === null || val === undefined) return "";
        if (typeof val === 'object' && val.id && val.title) {
            if (context === 'server') {
                return `<a href="/wiki/${val.id}" class="wiki-link">${val.title}</a>`;
            }
        ...
        pages: (source) => {
            ...
            if (typeof source === 'string') {
                if (source.startsWith('#')) {
                    ...
                    return db.prepare("SELECT * FROM objects WHERE class = ?")
                             .all(source)
                             .map(parseMetadata);
                }
            }
