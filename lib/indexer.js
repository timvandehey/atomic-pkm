...
export function updateFile(id) {
    ...
    try {
        ...
        if (metadata.type && !metadata.class) {
        ...
        for (const [key, value] of Object.entries(metadata)) {
            ...
            if (type === 'string' && key !== 'class' && key !== 'type' && key !== 'title' && key !== 'tags') {
        ...
        db.transaction(() => {
            db.prepare(`
                INSERT INTO objects (id, title, class, metadata, content, date, smart_text)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            // Update FTS Index
            db.prepare("DELETE FROM objects_fts WHERE id = ?").run(id);
            db.prepare("INSERT INTO objects_fts (id, title, content) VALUES (?, ?, ?)")
              .run(id, metadata.title || id, smartText);
...
export function deleteFile(id) {
    try {
        db.transaction(() => {
            // Check inside transaction to be thread-safe/atomic
            const existing = db.prepare("SELECT id FROM objects WHERE id = ?").get(id);
            if (!existing) return;

            db.prepare("DELETE FROM objects WHERE id = ?").run(id);
            db.prepare("DELETE FROM objects_fts WHERE id = ?").run(id);
            console.log(`Removed from DB: ${id}.md`);
        })();
    } catch (err) {
